...
__all__ = ['to_hhmmss', 'to_secs', 'display_video', 'check_resolution', 'check_fps', 'crop_video', 'overlay_video',
           'play_audio', 'change_audio_format', 'trim_audio', 'change_volume', 'loop_audio', 'concat_audios']
...
def _extract_audio(video):
    video = Path(video)
    ext = video.suffix
    mp4_version = AudioSegment.from_file(f"{video}", f"{ext[1:]}")
    mp4_version.export("temp_audio.mp3", format="mp3")
    return "temp_audio.mp3"

def _extract_video(video_path):
    return _get_frames(video_path)
...
def _get_frames(video_path):
    ...
    return file_list, fps

# Cell
def crop_video(video, coords, border_size = None, border_color = (255,0,0), outfile = "cropped_video.mp4"):
    """
    Crop a particular portion of the video. Additionally you can put a border around it

    Params:
    video: path to the video you want to crop
    coords: List / tuple of coords in the form [left, top, right, bottom]
    border_size: one of ["small", "medium", "large", None]
    border_color: RGB tuple
    outfile: name of the output file

    Returns:
    outfile: name of the output file
    """
    image_list, fps = _extract_video(video)
    op_audio = _extract_audio(video)

    assert len(coords) == 4, f"Expected 4 coords got {len(coords)}"
    left, top, right, bottom = coords

    border = border_size_map[border_size]

    width = right - left + border
    height = bottom - top + border

    # create a video writer with size > height width of the image frames by border
    out = cv2.VideoWriter('temp_result.avi', cv2.VideoWriter_fourcc(*'DIVX'), fps, (width, height))
    ...
    for i in tqdm(range(len(image_list))):
        ...
        image = cv2.resize(image.astype(np.uint8), (width - border, height - border), interpolation=cv2.INTER_LANCZOS4)

        background = np.zeros((height, width, 3), np.uint8)

        black = tuple(reversed((0,0,0)))
        background[:] = black
        background[2:-2,2:-2] = border_color

        border = border // 2
        if border != 0:
            background[border:-border,border:-border,:] = image
        else:
            background = image
    ...
    out.release()
    command = f'ffmpeg -y -i {op_audio} -i temp_result.avi -strict -2 -q:v 1 {outfile} -y'
    subprocess.call(command, shell = platform.system() != 'Windows')

    os.system(f'rm -rf temp_result.avi {op_audio}')
    return outfile


# Internal Cell
def _get_mapping_dict(od_x: int, od_y: int):
    overlay_mapping = {
...
def _get_len(file_path):
    file_len = subprocess.check_output(f'ffprobe -v error -show_entries format=duration -of default=noprint_wrappers=1:nokey=1 {file_path}', shell=True)
    return round(float(file_len), 2)


def _trim_video(video, end_time):
    video = Path(video)
    temp_trim = video.parent / (video.stem + str('temptrim') + video.suffix)
    os.system(f'cp {video} {temp_trim}')
    os.system(f'ffmpeg -ss 00:00:00 -i {temp_trim} -to {end_time} -c copy {video} -y')
...
def overlay_video(
    ...
    scale_x: int,
    scale_y: int,
    outfile: Union[Path, str] = "overlayed_video.mp4",
) -> str:
    """Overlay one video over another with the audio of the overlayed video
    Args:
        bk (Union[Path, str]): path of the background video
        ov (Union[Path, str]): path of the overlayed video
        outfile (Union[Path, str]): path of the output file
        pos (str): position to place the overlayed video, options [top-left, top-right, bottom-left, bottom-right]
        odist_x: (int) horizontal distance of the overlayed video from left or right based on pos
        odist_y: (int) vertical distance of the overlayed video from top or bottom based on pos
        scale_x (int): width of the overlayed video
        scale_y (int): height of the overlayed video
    Returns:
        str: path to the output file
    """
    overlay_dict = _get_mapping_dict(odist_x, odist_y)
    end_time = _get_len(ov)
    os.system(f'cp {bk} temp_background.mp4')
    _trim_video('temp_background.mp4', to_hhmmss(end_time))

    bk = "temp_background.mp4"

    ov, bk = Path(ov), Path(bk)

    cmd = f'ffmpeg -i {bk} -i {ov} -map 0:0 -map 1:1 -map 0:a -vf "movie={ov}, scale={scale_x}:{scale_y} [inner]; [in][inner] overlay={overlay_dict[pos]} [out]" {outfile} -y'
    try:
        op = subprocess.check_output(cmd, shell=True)
    except CalledProcessError as e:
        print(e.output)

    os.system('rm -rf temp_background.mp4')
    return outfile
